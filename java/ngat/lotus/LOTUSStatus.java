...
/**
 * This class holds status information for the LOTUS program.
 * @author Chris Mottram
 * @version $Revision: 1.1 $
 */
public class LOTUSStatus
{
	/**
	...
	/**
	 * Default filename containing network properties for LOTUS.
	 */
	private final static String DEFAULT_NET_PROPERTY_FILE_NAME = "./lotus.net.properties";
	/**
	 * Default filename containing properties for LOTUS.
	 */
	private final static String DEFAULT_PROPERTY_FILE_NAME = "./lotus.properties";
	/**
	 * Default filename containing FITS properties for LOTUS.
	 */
	...
	/**
	 * The current thread that the LOTUS Control System is using to process the
	...
	/**
	 * The current command that the LOTUS Control System is working on. This does not get set for
	 * commands that can be sent while others are in operation, such as Abort and get status comamnds.
	 * This can be null when no command is currently being processed.
	 */
	...
	/**
	 * A list of properties held in the properties file. This contains configuration information in LOTUS
	...
	/**
	 * The name of the ngat.phase2.LOTUSConfig object instance that was last used	
	 * to configure the instrument (via an ngat.message.ISS_INST.CONFIG message).
	 * Used for the CONFNAME FITS keyword value.
	 * Initialised to 'UNKNOWN', so that if we try to take a frame before configuring the LOTUS
	...
	/**
	 * Set the logging level for LOTUS.
	...
	/**
	 * Get the logging level for LOTUS.
	...
	/**
	 * Get the the command the LOTUS is currently processing.
	...
	/**
	 * Method to change (increment) the unique ID number of the last ngat.phase2.LOTUSConfig instance to 
	 * successfully configure the LOTUS camera.
	 ...
	 * ngat.phase2.LOTUSConfig instance to successfully configure the LOTUS camera.
	 ...
	 * ngat.phase2.LOTUSConfig instance to successfully configure the LOTUS camera.
	 * @return The string identifier, or null if the LOTUS camera has not been configured
	 * 	since the LOTUS started.
	 * @see #configName
	 */
	 ...
	 * The value is retrieved from the <b>lotus.thread.priority.server</b> property.
	 * If this fails the default LOTUS_DEFAULT_THREAD_PRIORITY_SERVER is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see LOTUSConstants#LOTUS_DEFAULT_THREAD_PRIORITY_SERVER
	 */
	public int getThreadPriorityServer()
	{
		...
		try
		{
			retval = getPropertyInteger("lotus.thread.priority.server");
		...
		{
			retval = LOTUSConstants.LOTUS_DEFAULT_THREAD_PRIORITY_SERVER;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>lotus.thread.priority.interrupt</b> property.
	 * If this fails the default LOTUS_DEFAULT_THREAD_PRIORITY_INTERRUPT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see LOTUSConstants#LOTUS_DEFAULT_THREAD_PRIORITY_INTERRUPT
	 */
	public int getThreadPriorityInterrupt()
	{
		...
		try
		{
			retval = getPropertyInteger("lotus.thread.priority.interrupt");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = LOTUSConstants.LOTUS_DEFAULT_THREAD_PRIORITY_INTERRUPT;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>lotus.thread.priority.normal</b> property.
	 * If this fails the default LOTUS_DEFAULT_THREAD_PRIORITY_NORMAL is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see LOTUSConstants#LOTUS_DEFAULT_THREAD_PRIORITY_NORMAL
	 */
	public int getThreadPriorityNormal()
	{
		...
		try
		{
			retval = getPropertyInteger("lotus.thread.priority.normal");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = LOTUSConstants.LOTUS_DEFAULT_THREAD_PRIORITY_NORMAL;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>lotus.thread.priority.tit</b> property.
	 * If this fails the default LOTUS_DEFAULT_THREAD_PRIORITY_TIT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see LOTUSConstants#LOTUS_DEFAULT_THREAD_PRIORITY_TIT
	 */
	public int getThreadPriorityTIT()
	{
		...
		try
		{
			retval = getPropertyInteger("lotus.thread.priority.tit");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = LOTUSConstants.LOTUS_DEFAULT_THREAD_PRIORITY_TIT;
		}
		return retval;
	}
	 ...
	 * This is got from the <i>lotus.config.unique_id_filename</i> property.
	...
	private void initialiseConfigId()
	{
		...
		fileName = getProperty("lotus.config.unique_id_filename");
