// LOTUSTCPServerConnectionThread.java
// $HeadURL$
package ngat.lotus;
...
/**
 * This class extends the TCPServerConnectionThread class for the LOTUS application.
 * @author Chris Mottram
 * @version $Revision: 61 $
 */
public class LOTUSTCPServerConnectionThread extends TCPServerConnectionThread
{
	/**
	 ...
	 * This can be retrieved from the LOTUS main object, which has a Hashtable with mappings between
	 * ngat.message class names and implementations of these commands.
	 * @see JMSCommandImplementation
	 * @see LOTUS#getImplementation
	 */
	 ...
	 */
	public LOTUSTCPServerConnectionThread(Socket connectionSocket)
	{
	 ...
	 * stored in the LOTUS object. It sets up the implementation objects references to the LOTUS main
	 ...
	 * @see LOTUS#getImplementation
	 * @see CommandImplementation#setLOTUS
	...
	// set the threads priority
		if(command instanceof INTERRUPT)
			this.setPriority(lotus.getStatus().getThreadPriorityInterrupt());
		else
			this.setPriority(lotus.getStatus().getThreadPriorityNormal());
	// get the implementation - this never returns null.
		commandImplementation = lotus.getImplementation(command.getClass().getName());
	...
	// initialises the command implementations response
		((CommandImplementation)commandImplementation).setLOTUS(lotus);
	 ...
	 * <li>If the command is not an interrupt command sub-class it calls the LOTUSStatus 
	 ...
	 * message class.
	 * <li>The LOTUSStatus is again updated to reflect this command/thread has finished processing. (If it's
	 ...
	 * @see LOTUS#log
	 * @see LOTUSStatus#setCurrentCommand
	 * @see LOTUSStatus#setCurrentThread
	 * @see #commandImplementation
	...
	// setup a generic done object until the command specific one is constructed.
		...
		if(command == null)
		{
			lotus.error("processCommand:command was null.");
			done.setErrorNum(LOTUSConstants.LOTUS_ERROR_CODE_BASE+100);
			done.setErrorString("processCommand:command was null.");
			done.setSuccessful(false);
			return;
		}
		lotus.log(Logging.VERBOSITY_VERY_TERSE,"Command:"+command.getClass().getName()+
			" Started.");
	...
	// This class of commands probably want to see what the current command is anyway.
		if(!(command instanceof INTERRUPT))
		{
			lotus.getStatus().setCurrentCommand((ISS_TO_INST)command);
			lotus.getStatus().setCurrentThread((Thread)this);
		}
	// setup return object.
		try
		{
	...
	// the status's current command is reset to null.
		catch(Exception e)
		{
			...
			lotus.error(s,e);
			done.setErrorNum(LOTUSConstants.LOTUS_ERROR_CODE_BASE+102);
			done.setErrorString(s+e);
			done.setSuccessful(false);
		}
	// change LOTUS status once command has been done
		if(!(command instanceof INTERRUPT))
		{
			lotus.getStatus().setCurrentCommand(null);
			lotus.getStatus().setCurrentThread(null);
		}
	// log command/done
		lotus.log(Logging.VERBOSITY_VERY_TERSE,"Command:"+command.getClass().getName()+
			  " Completed.");
		lotus.log(Logging.VERBOSITY_TERSE,"Done:"+done.getClass().getName()+
